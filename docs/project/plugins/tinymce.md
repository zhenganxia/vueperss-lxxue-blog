###  中文文档：
[http://tinymce.ax-z.cn/configure/content-formatting.php](http://tinymce.ax-z.cn/configure/content-formatting.php)
### 语言包下载：
[https://www.tiny.cloud/get-tiny/language-packages/](https://www.tiny.cloud/get-tiny/language-packages/)
## 使用版本
+ vue2："tinymce": "^5.3.0"  ，"@tinymce/tinymce-vue": "^3.0.1",
+ vue3："tinymce": "^6.1.0" ，"@tinymce/tinymce-vue": "^5.0.0"
```js
...
##  vue2使用方法
### 引入汉化包
<img :src="$withBase('/images/4a4559af98afad9abe7cdb1b5296d2f93716c882710209ae93c067a8b6a30821QzpcVXNlcnNcbGl1eHhcQXBwRGF0YVxSb2FtaW5nXERpbmdUYWxrXDU1OTc0NTkwOF92MlxJbWFnZUZpbGVzXDE2ODk5MjczOTU3NDdfNDQ3RTcxOUQtQjgyQy00MDNiLTk3MkQtMzM5NDBFOTQ5OTlELnBuZw==.png')" alt="foo">
...
:::tip
+ 注意事项：
+ 1.打包后找不到汉化包等public 需要处理引入路径： language_url: './tinymce/langs/zh-Hans.js',
+ 2.字数统计为汉语-处理为统计字节
init_instance_callback() {
        // 解决字数统计-wordcount只统计了中文字的字数，数字和字母被忽略问题 editor.getContainer()可以获取所有div
        if(props.plugins.indexOf('wordcount') > 0 ) {
            const lastDom = document.querySelector('button.tox-statusbar__wordcount');
            lastDom.click()
        }
    },
:::
#### 汉化包位置：
<img :src="$withBase('/images/1689927277515_28D68C61-366D-437a-AFED-7C184BD0D834.png')" alt="foo">
...
```vue
<template>
    <editor v-model="myValue" :init="init" :disabled="disabled" :id="tinymceId" ref="tinymceRef"></editor>
</template>
...
import 'tinymce/themes/silver/theme'
//在TinyMce.vue中接着引入相关插件-展示需要处理下props.plugins参数
...
const emits = defineEmits(["update:modelValue","on-validate"])
const props = defineProps({
    ...
    plugins: {
        type: [String, Array],
        default: "lists  table  image  wordcount ", // 限制字数添加ax_wordlimit 和maxlength配合使用
    },
    toolbar: {
        type: [String, Array],
        default:
...
const init = reactive({
    ...
    plugins: props.plugins,  // 这里有默认值-支持手动传参覆盖
    toolbar: props.toolbar, //这里的数据是在props里面就定义好了的
    font_formats: 'Arial=arial,helvetica,sans-serif; 宋体=SimSun; 微软雅黑=Microsoft Yahei; Impact=impact,chicago;', //字体
    fontsize_formats: '11px 12px 14px 16px 18px 24px 36px 48px 64px 72px', // 文字大小
    // paste_convert_word_fake_lists: false, // 插入word文档需要该属性
    paste_webkit_styles: "all",
    external_plugins: {  // 限制字数插件
        ax_wordlimit: './tinymce/plugins/ax_wordlimit/plugin.min.js'
    },
    ax_wordlimit_num: props.maxlength, // 限制字数
    ax_wordlimit_callback: function (editor, txt, num,validate) { // 限制字数回调
        console.log('当前字数：' + txt + '，限制字数：' + num,validate);
        ElMessage({
        type: "warning",
        message: `限制字数${num},请删除多余字数`
      });
    },
    ...
    images_upload_handler: (blobInfo) => new Promise((resolve, reject) => {
        if (blobInfo.blob().size / 1024 / 1024 > props.maxSize) {
            reject({ message: `上传失败，图片大小请控制在 ${props.maxSize}M以内`, remove: true })
            return
        } else {
            const img = 'data:image/jpeg;base64,' + blobInfo.base64()
            resolve(img)
...
### 封装组件使用方法：
```
<XdTinymce  v-model="addForm.value"></XdTinymce>
```
